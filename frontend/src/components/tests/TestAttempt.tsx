    ...
    {!started ? <TestInstructions title={title} questions={questions} duration={duration} languages={languages} name={name} onExit={exit} onStart={(file) => { setPhoto(file); deadline.current = Date.now() + duration * 60000; setStarted(true); requestFullscreen(); window.scrollTo(0, 0); }} /> : <>
      ...
      <main className="grid w-full gap-4 p-4 pb-28 sm:p-6 sm:pb-28 lg:grid-cols-[minmax(0,1fr)_300px]">
        <section className="min-w-0 rounded-2xl border border-line bg-white p-5 sm:p-8">
          ...
          {question ? <><div className="flex flex-wrap justify-between gap-2 text-xs text-ink-muted"><span>Question {index + 1} of {questions.length}</span><span>+{question.marks ?? 1} / −{question.negativeMarks ?? 0}</span></div><h2 className="mt-5 whitespace-pre-wrap text-lg font-semibold leading-7">{question.text}</h2><fieldset disabled={expired || saving} className="mt-6 space-y-3"><legend className="sr-only">Choose your answer</legend>{question.options.map((option, optionIndex) => <button type="button" aria-pressed={selected === optionIndex} key={optionIndex} onClick={() => setSelected((value) => value === optionIndex ? null : optionIndex)} className={`flex w-full items-start gap-3 rounded-xl border p-4 text-left text-sm disabled:opacity-60 ${selected === optionIndex ? 'border-brand-600 bg-brand-50' : 'border-line hover:bg-canvas'}`}><span className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full border ${selected === optionIndex ? 'border-brand-600' : 'border-slate-300'}`}>{selected === optionIndex && <span className="h-2 w-2 rounded-full bg-brand-600" />}</span><span className="font-semibold">{String.fromCharCode(65 + optionIndex)}.</span><span className="whitespace-pre-wrap">{option}</span></button>)}</fieldset><p className="mt-4 text-xs text-ink-muted">Click Save & Next or Mark for Review & Next to save your selection.</p><div className="mt-6 flex flex-wrap gap-2"><Button variant="secondary" disabled={expired || saving} onClick={() => setSelected(null)}>Clear response</Button>{review[question._id] && <Button variant="secondary" disabled={expired || saving} onClick={() => setReview((value) => ({ ...value, [question._id]: false }))}>Unmark</Button>}<Button variant="secondary" disabled={expired || saving} onClick={() => save(true)}>Mark for Review & Next</Button></div></> : <p className="text-sm text-ink-muted">No questions are available.</p>}
        </section>
