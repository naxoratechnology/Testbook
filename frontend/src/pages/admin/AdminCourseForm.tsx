...
type NewLecture = { subject: string; source: 'upload' | 'youtube'; youtubeUrl: string; name: string; description: string; video: File | null; pdf: File | null; isPreview: boolean };
...
export function AdminCourseForm() {
  ...
  const [existingLecturePreviews, setExistingLecturePreviews] = useState<Record<string, boolean>>({});
  ...
  useEffect(() => {
    if (current && current._id === courseId && loadedId.current !== courseId) { loadedId.current = courseId; setSubjects(current.subjects || []); setUseSubjects(Boolean(current.subjects?.length)); setExistingLectureSubjects(Object.fromEntries(current.lectures.map((lecture) => [lecture._id, lecture.subject || '']))); setExistingLecturePreviews(Object.fromEntries(current.lectures.map((lecture) => [lecture._id, Boolean(lecture.isPreview)]))); setCourse({ title: current.title, description: current.description, exam: current.exam, category: current.category, instructor: current.instructor, thumbnail: current.thumbnail || '', access: current.access, price: String(current.price), status: current.status }); }
  }, [current, courseId]);

  const update = (key: keyof FormState, value: string) => setCourse((item) => ({ ...item, [key]: value }));
  const addLecture = () => setLectures((items) => [...items, { subject: '', source: 'upload', youtubeUrl: '', name: '', description: '', video: null, pdf: null, isPreview: false }]);
  ...
  const selectedDemoCount = (current?.lectures || []).filter((lecture) => existingLecturePreviews[lecture._id] ?? lecture.isPreview).length + lectures.filter((lecture) => lecture.isPreview).length;
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (course.access === 'paid' && selectedDemoCount > 2) throw new Error('Choose no more than two free demo lectures.');
    ...
    const payload: CoursePayload = { ...course, subjects: selectedSubjects, price: course.access === 'free' ? 0 : Number(course.price), lectures: (editing || savedId.current) ? (current?.lectures || []).map((lecture) => ({ ...lecture, subject: useSubjects ? (existingLectureSubjects[lecture._id] ?? lecture.subject ?? '').trim() : '', isPreview: course.access === 'paid' && (existingLecturePreviews[lecture._id] ?? lecture.isPreview) })) : [] };
    ...
    for (const lecture of lectures) {
      await dispatch(addCourseLecture({ courseId: saved._id, lecture: { subject: useSubjects ? lecture.subject.trim() : '', title: lecture.name, description: lecture.description, video: lecture.video, youtubeUrl: lecture.youtubeUrl, pdf: lecture.pdf, isPreview: course.access === 'paid' && lecture.isPreview } })).unwrap();
    }
    ...
    <form onSubmit={(event) => { submit(event).catch(() => undefined); }} className="space-y-5">
      ...
      <Panel><div className="flex items-center justify-between"><div><h2 className="text-base font-semibold text-ink">Lectures</h2><p className="mt-1 text-[13px] text-ink-muted">Choose an uploaded video or a YouTube URL for each lecture, with optional PDF notes.</p></div><Button type="button" variant="secondary" size="sm" onClick={addLecture}><PlusIcon className="h-4 w-4" /> Add lecture</Button></div>
        ...
        <div className="mt-4 space-y-3">{!current?.lectures.length && lectures.length === 0 && <p className="rounded-xl bg-canvas px-4 py-8 text-center text-sm text-ink-muted">No lectures yet. Click Add lecture.</p>}{lectures.map((lecture, index) => <div key={index} className="rounded-xl border border-line p-4"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-semibold text-ink">New lecture {index + 1}</p><button type="button" onClick={() => setLectures((items) => items.filter((_item, i) => i !== index))} className="text-ink-muted hover:text-red-600"><Trash2Icon className="h-4 w-4" /></button></div><div className="grid gap-3 sm:grid-cols-2">
          ...
          {course.access === 'paid' && <label className="flex items-center gap-2 text-xs text-ink-soft sm:col-span-2"><input type="checkbox" checked={lecture.isPreview} disabled={selectedDemoCount >= 2 && !lecture.isPreview} onChange={(event) => updateLecture(index, { isPreview: event.target.checked })} />Free demo lecture (available before purchase)</label>}
        </div></div>)}</div>
      </Panel>
