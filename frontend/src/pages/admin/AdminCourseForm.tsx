...
type NewLecture = { source: 'upload' | 'youtube'; youtubeUrl: string; name: string; description: string; video: File | null; pdf: File | null };
...
export function AdminCourseForm() {
  ...
  useEffect(() => {
    if (current && current._id === courseId && loadedId.current !== courseId) { loadedId.current = courseId; setCourse({ title: current.title, description: current.description, exam: current.exam, category: current.category, instructor: current.instructor, thumbnail: current.thumbnail || '', access: current.access, price: String(current.price), status: current.status }); }
  }, [current, courseId]);

  const update = (key: keyof FormState, value: string) => setCourse((item) => ({ ...item, [key]: value }));
  const addLecture = () => setLectures((items) => [...items, { source: 'upload', youtubeUrl: '', name: '', description: '', video: null, pdf: null }]);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const payload: CoursePayload = { ...course, price: course.access === 'free' ? 0 : Number(course.price), lectures: (editing || savedId.current) ? current?.lectures || [] : [] };
    ...
    for (const lecture of lectures) {
      await dispatch(addCourseLecture({ courseId: saved._id, lecture: { title: lecture.name, description: lecture.description, video: lecture.video, youtubeUrl: lecture.youtubeUrl, pdf: lecture.pdf } })).unwrap();
    }
    ...
    <form onSubmit={(event) => { submit(event).catch(() => undefined); }} className="space-y-5">
      ...
      <Panel><div className="flex items-center justify-between"><div><h2 className="text-base font-semibold text-ink">Lectures</h2><p className="mt-1 text-[13px] text-ink-muted">Choose an uploaded video or a YouTube URL for each lecture, with optional PDF notes.</p></div><Button type="button" variant="secondary" size="sm" onClick={addLecture}><PlusIcon className="h-4 w-4" /> Add lecture</Button></div>
        ...
        <div className="mt-4 space-y-3">{!current?.lectures.length && lectures.length === 0 && <p className="rounded-xl bg-canvas px-4 py-8 text-center text-sm text-ink-muted">No lectures yet. Click Add lecture.</p>}{lectures.map((lecture, index) => <div key={index} className="rounded-xl border border-line p-4"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-semibold text-ink">New lecture {index + 1}</p><button type="button" onClick={() => setLectures((items) => items.filter((_item, i) => i !== index))} className="text-ink-muted hover:text-red-600"><Trash2Icon className="h-4 w-4" /></button></div><div className="grid gap-3 sm:grid-cols-2">
          <Input required value={lecture.name} onChange={(e) => updateLecture(index, { name: e.target.value })} placeholder="Lecture name" /><Select aria-label="Lecture video source" value={lecture.source} onChange={(e) => updateLecture(index, { source: e.target.value as 'upload' | 'youtube', video: null, youtubeUrl: '' })}><option value="upload">Upload video</option><option value="youtube">YouTube URL</option></Select>
