...
export function AdminTestBuilder() {
  ...
  const [title, setTitle] = useState(''); const [duration, setDuration] = useState('60'); const [status, setStatus] = useState<SeriesStatus>('draft');
  const [questions, setQuestions] = useState<DraftQuestion[]>(() => [blankQuestion()]);
  useEffect(() => { loaded.current = ''; setFailure(''); setTitle(''); setDuration('60'); setStatus('draft'); setQuestions([blankQuestion()]); }, [seriesId, testId]);
  useEffect(() => { if (seriesId) void dispatch(fetchAdminSeries(seriesId)); }, [seriesId, dispatch]);
  useEffect(() => {
    if (!test || loaded.current === test._id) return;
    loaded.current = test._id; setTitle(test.title); setDuration(String(test.duration)); setStatus(test.status);
  ...
  const validQuestions = questions.length > 0 && questions.every((question) => question.text.trim() && question.options.length >= 2 && question.options.every((option) => option.trim()) && question.correct >= 0 && question.correct < question.options.length && question.marks !== '' && Number.isFinite(Number(question.marks)) && Number(question.marks) >= 0 && question.negative !== '' && Number.isFinite(Number(question.negative)) && Number(question.negative) >= 0);
  const save = async () => {
    if (!seriesId || !validInformation || !validQuestions || saving) return;
    setFailure('');
    const payload = { title: title.trim(), duration: Number(duration), status, questions: questions.map((question) => ({ ...(question._id ? { _id: question._id } : {}), text: question.text.trim(), options: question.options.map((option) => option.trim()), correctAnswer: question.correct, explanation: question.explanation.trim(), marks: Number(question.marks), negativeMarks: Number(question.negative) })) };
  ...
  return <PageShell title={editing ? 'Edit test & questions' : 'Add test'} subtitle={`Test series: ${series.title}`} width="max-w-5xl">
    ...
    <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); void save(); }}>
      ...
      <Panel><h2 className="font-semibold text-ink">Test information</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Test name" className="sm:col-span-2"><Input required value={title} onChange={(event) => setTitle(event.target.value)} /></Field><Field label="Duration (minutes)"><Input required type="number" min="1" step="1" value={duration} onChange={(event) => setDuration(event.target.value)} /></Field><Field label="Status"><Select value={status} onChange={(event) => setStatus(event.target.value as SeriesStatus)}><option value="draft">Draft</option><option value="published">Published</option><option value="unpublished">Unpublished</option></Select></Field></div>{editing && test?.status === 'published' && <p className="mt-3 text-xs text-ink-muted">You can add or edit questions without unpublishing this test. Saved changes are visible to students.</p>}</Panel>
      ...
      {questions.map((question, index) => <Panel key={question.id}><div id={`question-${question.id}`} className="scroll-mt-24"><div className="flex items-center justify-between"><h3 className="font-semibold text-ink">Question {index + 1}</h3><ActionMenu actions={[{ label: 'Delete question', icon: <Trash2Icon className="h-4 w-4" />, danger: true, disabled: questions.length === 1 || saving, onClick: () => { if (!question._id || window.confirm('Delete this question when you save the test?')) setQuestions((items) => items.filter((item) => item.id !== question.id)); } }]} /></div><div className="mt-4 space-y-4"><Field label="Question"><Textarea required rows={3} value={question.text} onChange={(event) => update(question.id, { text: event.target.value })} /></Field><div className="grid gap-3 sm:grid-cols-2">{question.options.map((option, optionIndex) => <Field key={optionIndex} label={`Option ${String.fromCharCode(65 + optionIndex)}`}><div className="flex items-center gap-2"><Input required value={option} onChange={(event) => update(question.id, { options: question.options.map((item, i) => i === optionIndex ? event.target.value : item) })} /><button type="button" aria-pressed={question.correct === optionIndex} onClick={() => update(question.id, { correct: optionIndex })} aria-label={`Mark option ${String.fromCharCode(65 + optionIndex)} correct`} className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border ${question.correct === optionIndex ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-line text-ink-muted'}`}><CheckIcon className="h-4 w-4" /></button></div></Field>)}</div><p className="text-xs text-ink-soft">Correct answer: <Badge tone="green">{String.fromCharCode(65 + question.correct)}</Badge> Click the check beside the correct option.</p><Field label="Explanation"><Textarea rows={3} value={question.explanation} onChange={(event) => update(question.id, { explanation: event.target.value })} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Marks"><Input required type="number" min="0" step="0.25" value={question.marks} onChange={(event) => update(question.id, { marks: event.target.value })} /></Field><Field label="Negative marks"><Input required type="number" min="0" step="0.25" value={question.negative} onChange={(event) => update(question.id, { negative: event.target.value })} /></Field></div></div></div></Panel>)}
