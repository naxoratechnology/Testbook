...
export function AdminPreviousPapers() {
  ...
  return <PageShell title={directory ? directory + ' Previous Papers' : 'Previous Year Paper Collections'} subtitle="Organize papers into collections. One shared pass unlocks all PDFs and online tests." width="max-w-[1400px]" actions={directory ? <Link to={'/admin/previous-papers/new?directory=' + encodeURIComponent(directory)} className={btn('primary', 'md')}><PlusIcon className="h-4 w-4" /> Add Previous Paper</Link> : <>
    ...
    </> : <><Link to="/admin/previous-papers" className="mb-5 inline-block text-sm text-brand-700">← All collections</Link><TableWrap footer={loading ? 'Loading papers...' : `${items.filter((paper) => (paper.directory || paper.exam) === directory).length} papers`}><Table><thead><tr><Th>Paper</Th><Th>Exam</Th><Th>Year</Th><Th>Shift</Th><Th>Online test</Th><Th>Status</Th><Th className="text-right">Actions</Th></tr></thead><tbody>{!loading && !items.some((paper) => (paper.directory || paper.exam) === directory) && <tr><td colSpan={7} className="border-b border-line px-5 py-10 text-center text-sm text-ink-muted">No previous papers uploaded yet.</td></tr>}{items.filter((paper) => (paper.directory || paper.exam) === directory).map((paper) => <tr key={paper._id} className="hover:bg-canvas/60"><Td><p className="font-medium text-ink">{paper.title}</p><p className="text-xs text-ink-muted">{paper.stage} · {paper.subject}</p></Td><Td>{paper.exam}</Td><Td>{paper.year}</Td><Td>{paper.shift || '—'}</Td><Td>{paper.questions.length ? <Badge tone="brand">{paper.questions.length} Q · {paper.duration} min</Badge> : <Badge tone="slate">PDF only</Badge>}</Td><Td><StatusBadge status={paper.status} /></Td><Td><RowActions onView={() => window.open(paper.pdfUrl, '_blank', 'noopener,noreferrer')} onEdit={() => navigate(`/admin/previous-papers/${paper._id}/edit`)} onDelete={() => remove(paper._id)} extra={<button type="button" disabled={saving} onClick={() => toggle(paper)} className={btn('secondary', 'sm', 'mr-1')}>{paper.status === 'published' ? 'Unpublish' : 'Publish'}</button>} /></Td></tr>)}</tbody></Table></TableWrap></>}
