...
export function TestSeriesDetail() {
  ...
  return <PageShell><Link to="/test-series" className="text-sm text-brand-700">&larr; Test Series</Link><TestSeriesThumbnail src={series.thumbnail} alt={series.title} className="mt-5 h-48 w-full rounded-xl object-cover sm:h-64" /><div className="mt-5 flex gap-2"><Badge tone="brand">{series.exam}</Badge><Badge tone={series.access === 'free' ? 'green' : 'violet'}>{series.access === 'free' ? 'Free' : `₹${series.price}`}</Badge>{series.purchased && series.access === 'paid' && <Badge tone="green">Purchased</Badge>}</div><h1 className="mt-3 text-3xl font-bold text-ink">{series.title}</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-ink-soft">{series.description}</p>
    ...
    {error && <p className="mt-5 rounded-xl bg-red-50 p-4 text-sm text-red-700">{error}</p>}<Panel className="mt-6" padded={false}><div className="border-b border-line px-5 py-4"><h2 className="font-semibold text-ink">All Tests</h2></div><ul className="divide-y divide-line">{series.tests.map((test) => <li key={test._id} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-ink">{test.title}</p><p className="mt-1 text-xs text-ink-muted">{test.questions.length} questions · {test.duration} minutes</p></div>{unlocked ? <div className="flex flex-wrap gap-2"><button onClick={() => navigate(`/test-series/${series._id}/tests/${test._id}${attemptedTests.some((item) => item.sourceId === series._id && item.testId === test._id) ? '?reattempt=1' : ''}`)} className={btn('primary', 'sm')}>{attemptedTests.some((item) => item.sourceId === series._id && item.testId === test._id) ? 'Reattempt Test' : 'Start Test'}</button><SolutionLink reference={{ source: 'test-series', sourceId: series._id, testId: test._id }} attempted={attemptedTests.some((item) => item.sourceId === series._id && item.testId === test._id)} /></div> : <button disabled={paying} onClick={purchase} className={btn('secondary', 'sm')}><LockIcon className="mr-1 h-3.5 w-3.5" />Purchase required</button>}</li>)}</ul></Panel>
  </PageShell>;
}
