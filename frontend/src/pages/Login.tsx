...
export function Login() {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    login(identifier, password).then((role) => { const redirect = new URLSearchParams(location.search).get('redirect'); navigate(role === 'admin' ? '/admin/dashboard' : redirect || '/dashboard'); }).catch((e: Error) => setError(e.message)).finally(() => setLoading(false));
  };
